...
fluidPage(br(),
  HTML('<!DOCTYPE html>
       ...
       <h1 style="font-family:Copperplate Gothic Bold;letter-spacing:8px;text-align:center;"><i>e</i>PIC</h1>
       
       <h4 style = "font-family:Charlesworth;text-align:center;color:navy;"><em> Polymorphism calculator for molecular markers </em> </h4>        
...
ls2li<- function(list){
  ...
  out[1]<- paste(length(list),length(strsplit(strsplit(list[[1]], " ")[[1]], "")[[1]]), sep=" ")
  for(i in 1:length(list)){
    out[(i+1)]<- paste(names(list)[i], strsplit(list[[i]], " ")[[1]], sep=" ")
  }
  return(out)
}
...
<br></br>
<center><p> `r icon("home", lib = "glyphicon")` ***e*PIC** calculates the polymorphism indices as defined in *ePIC: An online polymorphism calculator for molecular markers.* Applications in Plant Science (*submitted*).</p></center>


<center> <h3>**INPUT**</h3> </center>

##### To start calculating, please provide your [`Phylip`](http://evolution.genetics.washington.edu/phylip.html) file. This should result in updated **data** similar to the template. 

`r icon("exclamation-sign", lib = "glyphicon")` If the **data** did not appear or does not look like exisiting example, please check your file and upload again (for example you can test the program with these `r downloadLink('biT', 'binary')` or `r downloadLink('coT', 'codominant')` `Phylip` toy files).
```{r, echo=FALSE}
